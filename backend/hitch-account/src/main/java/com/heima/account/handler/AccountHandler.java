...
public class AccountHandler {
    ...
    public ResponseVO<AccountVO> modifyPassword(AccountVO accountVO) {
        ...
        if (!CommonsUtils.encodeMD5(accountVO.getPassword()).equals(accountPO.getPassword())) {
            throw new BusinessRuntimeException(BusinessErrors.PARAM_CANNOT_EMPTY, "旧密码错误");
        }
        //新密码加密，对比旧密码，不允许相同
        if (CommonsUtils.encodeMD5(accountVO.getNewPassword()).equals(accountPO.getPassword())) {
            throw new BusinessRuntimeException(BusinessErrors.PARAM_CANNOT_EMPTY, "新密码不能与旧密码相同");
        }
        //校验通过，将新密码写入数据库，修改成功
